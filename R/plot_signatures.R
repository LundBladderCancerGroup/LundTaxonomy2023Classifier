...
#' @param subtype_annotation Can be one of the following; "5 class" (default) or "7 class" annotation.
...
#' Note, the title of the plot will always be twice that of the set value here (default = 10).
#' @param plot_font_row_size Optional parameter to control the size of the font in the generated heatmap.
#' Note, the title of the plot will always be twice that of the set value here (default = 8).
...
plot_signatures = function(these_predictions = NULL,
  ...
  #7 classes
  if(subtype_annotation == "7 classes"){
    ...
    if(plot_scores){
      ...
      bar7 = anno_barplot(as.numeric(score_matrix[,"ScNE"]), ylim = c(0, 1), gp = gpar(fill = lund_colors$lund_colors["ScNE"], border = TRUE, col = NA), bar_width = 1, height = unit(6, "mm"))
    }else{
      bar0 = NULL
      bar1 = NULL
      bar2 = NULL
  ...
  #5 classes
  }else if(subtype_annotation == "5 classes"){
