...
#'
#' @description Construct a forest plot using Cox model survival data for a set of signature scores.
#'
#' @details This function internally calls `get_survival`, which internally calls `int_prediction_wrangler`. 
#' This happens if the user does not call the plotting function using the `this_surv` parameter. 
...
#' `subtype_class`. To do so, set `all_subs = TRUE`. This will trigger a loop for which `get_survival` 
...
#'  in the docs.
#'
#' @param these_predictions Required parameter if `this_surv` is not provided. Should be output from 
#' [LundTax2023Classifier::lundtax_predict_sub()].
#' @param these_samples_metadata Required parameter if `this_surv`is not provided. Metadata associated
...
#' @param this_surv Required parameter if `these_predictions` and `these_samples_metadata` is not 
#' provided (The output from `get_survival`).
...
#' 
#' #build plot                                        
#' plot_hazard_forest(these_predictions = sjodahl_predicted,
...
plot_hazard_forest = function(these_predictions = NULL,
  ...
  if(!is.null(these_predictions) && is.null(this_surv)){
    if(!all_subs){
      ...
      if(subtype_class == "5_class"){
        this_surv = rbind(uro_surv, gu_surv, basq_surv, scne_surv, mes_surv)
      }else if(subtype_class == "7_class"){
        this_surv = rbind(uroa_surv, urob_surv, uroc_surv, gu_surv, basq_surv, scne_survs, mes_surv)
      }
    }
  }else if(is.null(these_predictions) && !is.null(this_surv)){
    message("Both predictions and survivor object are provided, function will plot the provided survivor data...")
  }
  ...
  if(!is.null(out_path)){
    #set PDF outputs
    if(out_format == "pdf"){
      pdf(paste0(out_path, plot_title, "_hazard_forest.pdf"),
          width = plot_width,
          height = plot_height)
      #set PNG outputs
    }else if(out_format == "png"){
      png(paste0(out_path, plot_title, "_hazard_forest.png"),
          width = plot_width,
          height = plot_height,
          units = "in",
          res = 300,
    ...
    print(my_plot)
    dev.off()
    message(paste0("Plot exported to ", out_path, plot_title, "_hazard_forest.", out_format))
  }else{
    return(my_plot) 
  }
}
